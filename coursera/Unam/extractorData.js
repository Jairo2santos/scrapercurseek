...
async function scrapeCourseraUNAM() {
  ...
  const linksFilePath = path.join(__dirname, 'linksUnam.txt'); // Asegúrate de que la ruta sea correcta
  ...
  await browser.close();
  await fs.writeFile('coursera_unam.json', JSON.stringify(courseDetails, null, 2));
