...
async function scrapeCourseraUNAM() {
  ...
  const linksFilePath = path.join(__dirname, 'linksUchile.txt'); // Asegúrate de que la ruta sea correcta
  ...
  for (let i = 0; i < courseLinks.length; i++) {
    ...
    const details = await page.evaluate(() => {
      ...
      if (hasRating) {
        ...
        level = document.querySelector("div:nth-child(2) > div > div.cds-119.cds-Typography-base.css-h1jogs.cds-121")?.innerText || "No level found";
        ...
        level = document.querySelector("section > div.css-lt1dx1 > div:nth-child(1) > div.cds-119.cds-Typography-base.css-h1jogs.cds-121")?.innerText || "No level found";
  ...
  await browser.close();
  await fs.writeFile('coursera_Uchile.json', JSON.stringify(courseDetails, null, 2));
