...
async function scrapeCourseraUNAM() {
  ...
  const linksFilePath = path.join(__dirname, 'linksUDeLosAndresCo.txt'); // Asegúrate de que la ruta sea correcta
  ...
  await browser.close();
  await fs.writeFile('coursera_UdeLosAndes.json', JSON.stringify(courseDetails, null, 2));
