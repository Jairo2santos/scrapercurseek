...
async function scrapeCourseraUNAM() {
  ...
  const linksFilePath = path.join(__dirname, 'linksUpChile.txt'); // Asegúrate de que la ruta sea correcta
  ...
  await browser.close();
  await fs.writeFile('coursera_UpChile.json', JSON.stringify(courseDetails, null, 2));
